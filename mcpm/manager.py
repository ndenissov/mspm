...
from .const import SOURCE_ICONS, DEFAULT_CONFIG
...
class PackageManager:
    def __init__(self, config_path="mpm.toml", auto_confirm=False, allow_untested_global=False, debug=False):
        ...
        self.client = httpx.AsyncClient(
            headers={"User-Agent": "MCPM/2.0"}, follow_redirects=True, timeout=30.0
        )

        # --- NEW: Concurrency Control ---
        self.browser_lock = asyncio.Lock()  # Только один браузер за раз
        self.cached_cookies = {}  # Кэш кук (cf_clearance)
        self.cached_ua = None
        # --------------------------------

        if not self.config_path.exists():
            with open(self.config_path, "w") as f: f.write(DEFAULT_CONFIG)
    ...
    def _save_state(self):
        if "dependencies" in self.config:
            self.config["dependencies"] = dict(sorted(self.config["dependencies"].items(), key=lambda i: i[0].lower()))
    ...
    def check_compatibility(self, name, supported_versions, source_name) -> bool:
        ...
        # FIX: Find real key (case insensitive) to check allow_untested
    ...
    async def process_plugin(self, name, spec, mode="install"):
        res_dict = {"name": name, "source": spec.get("source", "?"), "status": "resolving", "msg": "...",
                    "lock_data": None}

        if self.debug: console.log(f"[dim]Processing {name} ({mode})...[/]")
    ...
    async def _download_file(self, url, dest, info):
        ...
        if info.get("source") == "spigot":
            # 1. Try Cached Browser Cookies First
            if self.cached_cookies:
                if await self._try_download_with_cookies(url, dest, self.cached_cookies, self.cached_ua):
                    return True
                # If cached cookies failed, maybe they expired. Clear them.
                if self.debug: console.log(f"[yellow]Cached cookies expired for {info.get('real_name')}.[/]")
                self.cached_cookies = {}

            # 2. Try Automated Scraper
            if USING_SCRAPER:
                try:
                    ...
                    if "text/html" in r.headers.get("Content-Type", "") and r.status_code == 200:
                        raise Exception("Cloudflare blocked (HTML)")
                    r.raise_for_status()
            ...
            # 3. Browser Fallback (With Lock)
            # Only one browser at a time!
            async with self.browser_lock:
                # Re-check cache in case another thread filled it while we waited
                if self.cached_cookies:
                    if await self._try_download_with_cookies(url, dest, self.cached_cookies, self.cached_ua):
                        return True

                # Launch Browser
                if self.debug: console.log(f"[yellow]Launching browser for {info.get('real_name')}...[/]")
        ...
        if self.debug:
            ...
            with Live(table, refresh_per_second=10) as live:
                ...
                for future in asyncio.as_completed(tasks):
                    ...
                    table.add_column("Plugin", width=30);
                    table.add_column("Status")
                    for k in sorted_keys: table.add_row(rows[k]["icon"], k, rows[k]["msg"])
                    live.update(table)

        # Save Results
        if "dependencies" not in self.lock_data: self.lock_data["dependencies"] = {}
        for name, res in results.items():
            if res.get("lock_data"): self.lock_data["dependencies"][name] = res["lock_data"]
        self._save_state()

        # Handle Compatibility Prompts
        if incompatible:
            ...
            for name, vers in incompatible:
                ...
                if Confirm.ask(f"Force install [cyan]{name}[/] anyway?"):
                    # FIX: Use correct key casing to save config
    ...
    # ... add_plugins, remove_plugins, search (без изменений) ...
        ...
        for name in names:
            ...
            if not source:
                ...
                if exact:
                    ...
                    console.print(f"[red]No exact match for {name}[/]")
                    continue
            else:
                entry = {"source": source, "id": name}
            if version: entry["version"] = version
            if "dependencies" not in self.config: self.config["dependencies"] = {}
    ...
    async def search(self, query):
        ...
        t.add_column("Name");
        t.add_column("ID")
        for r in res: t.add_row(r["source"], r["name"], r["id"])
        console.print(t)
