...
import httpx
import asyncio
from .const import SOURCE_PRIORITY
...
class ResolverEngine:
    ...
    async def search_query(self, query: str):
        ...
        async def search_modrinth():
            try:
                r = await self.client.get("https://api.modrinth.com/v2/search",
                                          params={"query": query, "limit": 5, "facets": '[["project_type:plugin"]]'})
                if r.status_code == 200:
                    for hit in r.json().get("hits", []):
                        results.append({"source": "modrinth", "name": hit["title"], "id": hit["slug"],
        ...
        async def search_hangar():
            try:
                r = await self.client.get("https://hangar.papermc.io/api/v1/projects", params={"q": query, "limit": 5})
                if r.status_code == 200:
                    for hit in r.json().get("result", []):
                        slug = hit.get("namespace", {}).get("slug", hit["name"])
                        results.append(
                            {"source": "hangar", "name": hit["name"], "id": slug, "desc": hit["description"][:60]})
            except:
                pass

        async def search_spigot():
            try:
                r = await self.client.get(f"https://api.spiget.org/v2/search/resources/{query}",
                                          params={"size": 5, "field": "name"})
                if r.status_code == 200:
                    for hit in r.json():
                        results.append({"source": "spigot", "name": hit["name"], "id": str(hit["id"]),
                                        "desc": hit.get("tag", "")[:60]})
            except:
                pass
        ...
        async def search_bukkit():
            try:
                params = {"gameId": 432, "classId": 5, "searchFilter": query, "sortField": 2, "sortOrder": "desc",
                          "pageSize": 5}
                ...
                if r.status_code == 200:
                    ...
                    if isinstance(hits, list):
                        for hit in hits:
                            results.append({"source": "bukkit", "name": hit["name"], "id": str(hit["id"]),
    ...
    async def discover_id(self, name, source):
        if source == "modrinth": return name.lower()
        if source == "hangar": return name
        if source == "spigot":
            try:
                r = await self.client.get(f"https://api.spiget.org/v2/search/resources/{name}",
                                          params={"field": "name", "size": 1})
                if r.json(): return str(r.json()[0]['id'])
            except:
                pass
        if source == "bukkit":
            try:
                r = await self.client.get("https://api.curse.tools/v1/cf/mods/search",
    ...
    async def resolve(self, name, spec, compat_checker):
        ...
        if source == "modrinth" and res_id: res_id = res_id.lower()

        if not res_id: res_id = await self.discover_id(name, source)
        if not res_id: return None

        if self.debug: console.log(f"[dim]Resolving {name} on {source} ID={res_id}[/]")

        if source == "modrinth": return await self._res_modrinth(name, res_id, spec.get("version"))
        if source == "hangar": return await self._res_hangar(name, res_id, spec.get("version"), compat_checker)
        if source == "spigot": return await self._res_spigot(name, res_id, spec.get("version"), compat_checker)
        if source == "bukkit": return await self._res_bukkit(name, res_id, spec.get("version"), compat_checker)
        return None
    ...
    async def _resolve_github(self, url):
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                if jars: return jars[0]["browser_download_url"]
        except:
            pass
        return url
    ...
    async def _res_modrinth(self, name, slug, req_ver):
        try:
            ...
            if p_r.status_code == 404:
                if self.debug: console.log(f"[red]Modrinth 404 for {slug}[/]")
                return None
            ...
            found_any = False
            for ver in versions:
                if req_ver and ver["version_number"] != req_ver: continue
                ...
                if not req_ver:
                    if self.server_version not in ver["game_versions"]:
                        if not any(gv.startswith(self.server_major) for gv in ver["game_versions"]): is_game_ok = False

                if not is_loader_ok or not is_game_ok:
                    found_any = True
                    if self.debug and not is_loader_ok: console.log(
                        f"[dim]Skip {ver['version_number']}: loader mismatch[/]")
                    if self.debug and not is_game_ok: console.log(
                        f"[dim]Skip {ver['version_number']}: version mismatch[/]")
                    continue
            ...
            if self.debug: console.log(f"Modrinth Ex: {e}")
        return None

    async def _res_hangar(self, name, slug, req_ver, compat_check):
        try:
            r = await self.client.get(f"https://hangar.papermc.io/api/v1/projects/{slug}/versions",
                                      params={"limit": 50})
            if r.status_code != 200:
                if self.debug: console.log(f"[red]Hangar 404/Error: {r.status_code}[/]")
                return None
            versions = r.json().get("result", [])

            found_any = False
            for ver in versions:
                if req_ver and ver["name"] != req_ver: continue
                if self.platform not in ver["downloads"]: continue

                deps = ver.get("platformDependencies", {}).get(self.platform, [])
                if not req_ver and deps and self.server_version not in deps:
                    found_any = True
                    if self.debug: console.log(f"[dim]Skip {ver['name']}: dep mismatch[/]")
                    continue
                ...
                return {"source": "hangar", "version": ver["name"], "real_name": name, "url": url, "filename": filename,
                        "hash": hash_val, "hash_algo": "sha256"}
    ...
    async def _res_spigot(self, name, resid, req_ver, compat_check):
        try:
            info = await self.client.get(f"https://api.spiget.org/v2/resources/{resid}",
                                         params={"fields": "testedVersions,name"})
            if info.status_code != 200: return None
            info_j = info.json()
            ...
            r = await self.client.get(f"https://api.spiget.org/v2/resources/{resid}/versions",
                                      params={"size": 10, "sort": "-releaseDate"})
            versions = r.json()
            if not versions: return None
            target = versions[0]
            if req_ver: target = next((v for v in versions if v["name"] == req_ver), target)
            return {"source": "spigot", "version": target["name"], "real_name": real_name,
                    "url": f"https://api.spiget.org/v2/resources/{resid}/versions/{target['id']}/download",
                    "filename": f"{name}-{target['name']}.jar", "hash": "", "hash_algo": "none", "spigot_res_id": resid,
    ...
    async def _res_bukkit(self, name, pid, req_ver, compat_check):
        try:
            ...
            for f in files:
                if req_ver and req_ver in f.get("displayName", ""): return self._fmt_bukkit(f, pid, real_name)
                if not req_ver:
                    if self.server_version in f["gameVersions"] or self.server_major in f[
                        "gameVersions"]: return self._fmt_bukkit(f, pid, real_name)
            latest = files[0]
    ...
    def _fmt_bukkit(self, f, pid, rname):
        h = next((i["value"] for i in f.get("hashes", []) if i["algo"] == 1), "")
        return {"source": "bukkit", "version": str(f["id"]), "real_name": rname, "url": f["downloadUrl"],
